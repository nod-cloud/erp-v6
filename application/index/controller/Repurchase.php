...
class Repurchase extends Acl {
        ...
        if(isset($input['id'])){
            ...
            if(isset_full($input,'tab')){
                foreach ($input['tab'] as $tab_key=>$tab_vo) {
                    $tab_vali = $this->validate($tab_vo,'Repurchaseinfo');//详情验证
                    if($tab_vali!==true){
            ...
            if(empty($input['id'])){
                ...
                $vali = $this->validate($input,'Repurchaseclass');
                if($vali===true){
                    $create_info=Repurchaseclass::create(syn_sql($input,'repurchaseclass'));
                    Hook::listen('create_repurchase',$create_info);//购货退货单新增行为
                    push_log('新增购货退货单[ '.$create_info['number'].' ]');//日志
                ...
                //更新
                $vali = $this->validate($input,'Repurchaseclass.update');
                if($vali===true){
                    $update_info=Repurchaseclass::update(syn_sql($input,'repurchaseclass'));
                    Hook::listen('update_repurchase',$update_info);//购货退货单更新行为
                    push_log('更新购货退货单[ '.$update_info['number'].' ]');//日志
                    Repurchaseinfo::where(['pid'=>$update_info['id']])->delete();
                    $resule=['state'=>'success'];
                }else{
                    $resule=['state'=>'error','info'=>$vali];
                }
            ...
            if($resule['state']=='success'){
                ...
                foreach ($input['tab'] as $info_vo) {
                    ...
                    Repurchaseinfo::create(syn_sql($info_vo,'repurchaseinfo'));
                }
            }
        }else{
        ...
        if(isset_full($input,'page') && isset_full($input,'limit')){
            ...
            if(isset_full($input,'name')){
                ...
                if(empty($goods)){
                    ...
                    $info=get_db_field('repurchaseinfo',['goods'=>['in',$goods]],'pid');//取出详情表数据
                    sql_assign($sql,'id',$info);//多表查询赋值处理
                }
            }
            //处理仓库搜索
            if(isset_full($input,'warehouse')){
                $info=get_db_field('repurchaseinfo',['warehouse'=>['in',explode(",",$input['warehouse'])]],'pid');//取出详情表数据
                sql_assign($sql,'id',$info,'intersect');//多表查询赋值处理
            }
            $sql['merchant']=['in',get_auth_merchant()];//补全授权商户数据
            $sql=auth('repurchaseclass',$sql);//数据鉴权
            $count = Repurchaseclass::where ($sql)->count();//获取总条数
            $arr = Repurchaseclass::with('merchantinfo,supplierinfo,userinfo,accountinfo')->where($sql)->page($input['page'],$input['limit'])->order('id desc')->select();//查询分页数据
            $resule=[
                'code'=>0,
                'msg'=>'获取成功',
                'count'=>$count,
    ...
    public function info(){
        ...
        if(isset_full($input,'id')){
            $class=Repurchaseclass::where(['id'=>$input['id']])->find();
            $info=Repurchaseinfo::with('roominfo,goodsinfo,warehouseinfo')->where(['pid'=>$input['id']])->select()->toarray();
    ...
    public function auditing($arr=[],$auto=false){
        ...
        if(empty($arr)){
            ...
            foreach ($arr as $arr_vo) {
                $class=Repurchaseclass::where(['id'=>$arr_vo])->find();
                $info=Repurchaseinfo::where(['pid'=>$arr_vo])->select();
                //判断操作类型
                if(empty($class['type']['nod'])){
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                $auto&&(push_log('自动审核购货退货单[ '.$class['number'].' ]失败,原因:第'.($info_key+1).'行串码状态不正确!'));//日志
                                return json(['state'=>'error','info'=>'审核-购货退货单[ '.$class['number'].' ]失败,原因:第'.($info_key+1).'行串码状态不正确!']);
                                exit;
                            }
                        }
                    }
                    ...
                    foreach ($info as $info_key=>$info_vo) {
                        if(!empty($info_vo['serial'])){
                            ...
                            if(!empty($serial)){
                                return json(['state'=>'error','info'=>'反审核-购货退货单[ '.$class['number'].' ]第'.($info_key+1).'行串码状态不正确!']);
                                exit;
                            }
                        }
                    }
            ...
            foreach ($arr as $arr_vo) {
                ...
                if(empty($class['type']['nod'])){
                    ...
                    foreach ($info as $info_vo) {
                        ...
                        if (!empty($info_vo['serial'])){
                            ...
                            foreach ($serial_arr as $serial_arr_vo) {
                                ...
                                Serial::update(['id'=>$serial['id'],'type'=>2]);
                                //新增串码详情
                                Serialinfo::create (['pid'=>$serial['id'],'type'=>3,'class'=>$arr_vo]);
                            }
                        }
                    }
                    //获取核销状态
                    ...
                    if (!empty($class['money'])){
                        //新增对账单
                        $bill=Repurchasebill::create(['pid'=>$arr_vo,'account'=>$class['account'],'money'=>$class['money'],'data'=>'系统自动生成','user'=>Session('is_user_id'),'time'=>time()]);
                        Account::where (['id'=>$class['account']])->setInc('balance',$class['money']);//操作资金账户[+]
                        Accountinfo::create (['pid'=>$class['account'],'set'=>1,'money'=>$class['money'],'type'=>3,'time'=>time(),'user'=>Session('is_user_id'),'class'=>$arr_vo,'bill'=>$bill['id']]);//新增资金详情
                    }
                    Repurchaseclass::update(['id'=>$arr_vo,'type'=>1,'auditinguser'=>Session('is_user_id'),'auditingtime'=>time(),'billtype'=>$billtype]);//更新CLASS数据
                    set_summary('repurchase',$arr_vo,true);//更新统计表
                    push_log(($auto?'自动':'').'审核购货退货单[ '.$class['number'].' ]');
                }else{
                    //反审核操作
                    foreach ($info as $info_vo){
                        ...
                        if(!empty($info_vo['serial'])){
                            ...
                            foreach ($serial as $serial_vo) {
                                ...
                                Serialinfo::where(['pid'=>$serial_vo['id'],'type'=>3,'class'=>$arr_vo])->delete();//删除串码详情
                            }
                        }
                    }
                    Roominfo::where(['type'=>3,'class'=>$arr_vo])->delete();//删除仓储详情
                    //操作核销信息
                    if (!empty($class['money'])){
                        $bill=Repurchasebill::where(['pid'=>$arr_vo])->select();
                        foreach ($bill as $bill_vo){
                            Account::where(['id'=>$bill_vo['account']])->setDec('balance',$bill_vo['money']);//操作资金账户[-]
                        }
                        Accountinfo::destroy (['type'=>3,'class'=>$arr_vo]);//删除资金详情
                        Repurchasebill::destroy(['pid'=>$arr_vo]);//删除对账单信息
                    }
                    Repurchaseclass::update(['id'=>$arr_vo,'type'=>0,'money'=>0,'auditinguser'=>0,'auditingtime'=>0,'billtype'=>-1]);//更新CLASS数据
                    set_summary('repurchase',$arr_vo,false);//更新统计表
                    push_log ('反审核购货退货单[ '.$class['number'].' ]');
                }
            }
            $resule=['state'=>'success'];
        }
    ...
    public function del(){
        ...
        if(isset_full($input,'arr') && is_array($input['arr'])){
            $class=db('repurchaseclass')->where(['id'=>['in',$input['arr']]])->select()->ToArray();//获取CLASS数据
            $data = searchdata($class,['type'=>['eq',1]]);//查询已审核单据
            //数据检验
            if(empty($data)){
                foreach ($class as $class_vo) {
                    push_log('删除购货退货单[ '.$class_vo['number'].' ]');//日志
                    Hook::listen('del_repurchase',$class_vo['id']);//购货退货单删除行为
                }
                Repurchaseclass::where(['id'=>['in',$input['arr']]])->delete();
                Repurchaseinfo::where(['pid'=>['in',$input['arr']]])->delete();
                $resule=['state'=>'success'];
            }else{
                $resule=['state'=>'error','info'=>'购货退货单[ '.$data[0]['number'].' ]已审核,不可删除!'];
            }
        }else{
            $resule=['state'=>'error','info'=>'传入参数不完整!'];
        }
    ...
    public function exports(){
        ...
        if(isset($input['mode'])){
            ...
            $sql['merchant']=['in',get_auth_merchant()];//补全授权商户数据
            $sql=auth('repurchaseclass',$sql);//数据鉴权
            $arr = Repurchaseclass::with('merchantinfo,supplierinfo,userinfo,accountinfo')->where($sql)->order('id desc')->select();//查询数据
            //判断报表类型
            if(empty($input['mode'])){
                //简易报表
                $formfield=get_formfield('repurchase_export','array');//获取字段配置
                //开始构造导出数据
                $excel=[];//初始化导出数据
                //1.填充标题数据
                array_push($excel,['type'=>'title','info'=>'购货退货单列表']);
                //2.构造表格数据
                $table_cell=[];//初始化表头数据
                //构造表头数据
                ...
                //4.导出execl
                export_excel('购货退货单列表',$excel);
            }else{
                //详细报表
                $files=[];//初始化文件列表
                $formfield=get_formfield('repurchase_exports','array');//获取字段配置
                //配置字段
                ...
                foreach ($arr as $arr_vo) {
                    $excel=[];//初始化导出数据
                    //1.填充标题数据
                    array_push($excel,['type'=>'title','info'=>'购货退货单']);
                    //2.添加基础字段
                    array_push($excel,['type'=>'node','info'=>[
                    ...
                    //3.构造表格数据
                    $info=Repurchaseinfo::where(['pid'=>$arr_vo['id']])->select();
                    $table_cell=[];//初始化表头数据
                    //构造表头数据
                    foreach ($formfield as $formfield_vo) {
                        $table_cell[$formfield_vo['key']]=$formfield_vo['text'];
                ...
                file_to_zip('购货退货单明细',$files);//打包输出数据
            }
        }else{
            $resule=['state'=>'error','info'=>'传入参数不完整!'];
        }
    ...
    public function prints(){
        $input=input('get.');
        if(isset_full($input,'id')){
            $print_name='repurchase';//模板标识
            $class=Repurchaseclass::where(['id'=>$input['id']])->find();
            $info=Repurchaseinfo::where(['pid'=>$input['id']])->select();
            $sys=get_sys(['enable_batch','enable_serial','print_paper']);
            //1.获取字段信息
            $formfield=get_formfield('repurchase_print','array');//获取字段配置
            //2.配置字段
            empty($sys['enable_batch'])&&(arrs_key_del($formfield,['key','batch']));
            empty($sys['enable_serial'])&&(arrs_key_del($formfield,['key','serial']));
            //3.构造表格数据
    ...
    public function bill_list(){
        ...
        if(isset_full($input,'page') && isset_full($input,'limit')){
            ...
            if(isset_full($input,'account')){
                $info=get_db_field('repurchasebill',['account'=>['in',explode(",",$input['account'])]],'pid');//取出表数据
                sql_assign($whereor,'id',$info);//多表查询赋值处理
            }
            $sql['type']=1;//补充条件
            $sql['merchant']=['in',get_auth_merchant()];//补全授权商户数据
            $sql=auth('repurchaseclass',$sql);//数据鉴权
            $count = Repurchaseclass::where($sql)->whereor($whereor)->count();//获取总条数
            $arr = Repurchaseclass::with('merchantinfo,supplierinfo,userinfo,accountinfo')->where($sql)->whereor($whereor)->page($input['page'],$input['limit'])->order('id desc')->select();//查询分页数据
            $resule=[
                'code'=>0,
                'msg'=>'获取成功',
                'count'=>$count,
    ...
    public function bill_info(){
        $input=input('post.');
        if(isset_full($input,'id')){
            $resule=Repurchasebill::with('accountinfo,userinfo')->where(['pid'=>$input['id']])->order('id desc')->select();
        }else{
            $resule=['state'=>'error','info'=>'传入参数不完整!'];
        }
        return json($resule);
    ...
    public function add_bill(){
        ...
        if(isset_full($input,'pid') && isset_full($input,'account') && isset_full($input,'money')){
            $class=Repurchaseclass::where(['id'=>$input['pid']])->find();//获取CLASS数据
            ...
            if($plus>$class['actual']){
                ...
                Repurchaseclass::where(['id'=>$input['pid']])->update(['billtype'=>$billtype,'money'=>$plus]);//更新CLASS数据
                //2.操作核销数据
                $input['time']=time();
                $input['user']=Session('is_user_id');
                $create_info=Repurchasebill::create(syn_sql($input,'repurchasebill'));
                Hook::listen('create_repurchasebill',$create_info);//购货退货核销单新增行为
                //3.操作资金账户
                Account::where (['id'=>$input['account']])->setInc('balance',$input['money']);//操作资金账户[+]
                Accountinfo::create (['pid'=>$input['account'],'set'=>1,'money'=>$input['money'],'type'=>3,'time'=>time(),'user'=>Session('is_user_id'),'class'=>$class['id'],'bill'=>$create_info['id']]);//新增资金详情
                //4.返回数据
                push_log('添加购货退货核销单信息[ '.$class['number'].' ]');//日志
                $bill=Repurchasebill::with('accountinfo,userinfo')->where(['id'=>$create_info['id']])->find();
    ...
    public function del_bill(){
        ...
        if(isset_full($input,'id')){
            //1.操作BILL数据
            $bill=Repurchasebill::where(['id'=>$input['id']])->find();//获取BILL数据
            Repurchasebill::where(['id'=>$input['id']])->delete();//删除BILL数据
            Hook::listen('del_repurchasebill',$bill);//购货退货核销单删除行为
            //2.操作资金账户
            Account::where(['id'=>$bill['account']])->setDec('balance',$bill['money']);//操作资金账户[-]
            Accountinfo::where(['type'=>3,'bill'=>$bill['id']])->delete();//删除资金详情
            //3.操作CLASS数据
            $class=Repurchaseclass::where(['id'=>$bill['pid']])->find();//获取CLASS数据
            $reduce=bcsub($class['money'],$bill['money'],config('decimal'));//初始化新金额[高精度]
            $billtype=($reduce>0)?1:0;//获取核销状态
            Repurchaseclass::where(['id'=>$bill['pid']])->update(['billtype'=>$billtype,'money'=>$reduce]);//更新CLASS数据
            //4.返回数据
            push_log('删除购货退货核销单信息[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'传入参数不完整!'];
        }
    ...
    public function force_bill(){
        ...
        if(isset_full($input,'id')){
            //1.操作CLASS数据
            $class=Repurchaseclass::where(['id'=>$input['id']])->find();//获取CLASS数据
            Repurchaseclass::where(['id'=>$input['id']])->update(['billtype'=>3]);//更新CLASS数据
            //2.返回数据
            push_log('强制核销购货退货核销单[ '.$class['number'].' ]');//日志
            $resule=['state'=>'success'];
        }else{
            $resule=['state'=>'error','info'=>'传入参数不完整!'];
        }
        ...
        if(isset_full($input,'arr') && isset_full($input,'account') && isset_full($input,'money')){
            ...
            foreach ($class as $class_vo){
                ...
                if(bccomp($money,0,config('decimal'))==0){
                    ...
                    Repurchaseclass::where(['id'=>$class_vo['id']])->update(['billtype'=>$billtype,'money'=>$plus]);//更新CLASS数据
                    //3.操作核销数据
                	$create_bill=Repurchasebill::create([
                	    'pid'=>$class_vo['id'],
                	    'account'=>$account,
                	    'money'=>$this_money,
                	    'data'=>$data,
                	...
                	Hook::listen('create_repurchasebill',$create_bill);//购货退货核销单新增行为
                	//4.操作资金账户
                	Account::where (['id'=>$account])->setInc('balance',$this_money);//操作资金账户[+]
                	Accountinfo::create (['pid'=>$account,'set'=>1,'money'=>$this_money,'type'=>3,'time'=>time(),'user'=>Session('is_user_id'),'class'=>$class_vo['id'],'bill'=>$create_bill['id'],'data'=>$data]);//新增资金详情
                	//5.更新数据
    ...
    public function bill_export(){
        ...
        $sql=auth('repurchaseclass',$sql);//数据鉴权
        $arr = Repurchaseclass::with('merchantinfo,supplierinfo,userinfo,accountinfo')->where($sql)->order('id desc')->select();//查询数据
        $formfield=get_formfield('repurchasebill_export','array');//获取字段配置
        //开始构造导出数据
        $excel=[];//初始化导出数据
        //1.填充标题数据
        array_push($excel,['type'=>'title','info'=>'购货退货核销单信息']);
        //2.构造表格数据
        $table_cell=[];//初始化表头数据
        //构造表头数据
        foreach ($formfield as $formfield_vo) {
        ...
        //4.导出execl
        push_log('导出购货退货核销单信息');//日志
        export_excel('购货退货核销单信息',$excel);
    }
}
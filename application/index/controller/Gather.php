...
class Gather extends Acl {
    ...
    public function set(){
        ...
        if(isset($input['id'])){
            ...
            if(empty($input['id'])){
                ...
                $vali = $this->validate($input,'Gatherclass');
                if($vali===true){
                ...
                //更新
                $vali = $this->validate($input,'Gatherclass.update');
                if($vali===true){
            ...
            if($resule['state']=='success'){
                ...
                foreach ($input['tab'] as $info_vo) {
                    ...
                    Gatherinfo::create(syn_sql($info_vo,'gatherinfo'));
                }
            }
        }else{
    ...
    public function form_list(){
        ...
        if(isset_full($input,'page') && isset_full($input,'limit')){
            ...
            if(isset_full($input,'account')){
                $info=get_db_field('gatherinfo',['account'=>['in',explode(",",$input['account'])]],'pid');//取出详情表数据
                sql_assign($sql,'id',$info,'intersect');//多表查询赋值处理
            }
            $sql['merchant']=['in',get_auth_merchant()];//补全授权商户数据
            $sql=auth('gatherclass',$sql);//数据鉴权
            $count = Gatherclass::where ($sql)->count();//获取总条数
    ...
    public function info(){
        ...
        if(isset_full($input,'id')){
            $class=Gatherclass::where(['id'=>$input['id']])->find();
            $info=Gatherinfo::with('accountinfo')->where(['pid'=>$input['id']])->select()->toarray();
    ...
    public function auditing($arr=[],$auto=false){
        ...
        if(empty($arr)){
            ...
            foreach ($arr as $arr_vo) {
                $class=Gatherclass::where(['id'=>$arr_vo])->find();
                $info=Gatherinfo::where(['pid'=>$arr_vo])->select();
    ...
    public function del(){
        ...
        if(isset_full($input,'arr') && is_array($input['arr'])){
            $class=db('gatherclass')->where(['id'=>['in',$input['arr']]])->select()->ToArray();//获取CLASS数据
    ...
    public function exports(){
        ...
        if(isset($input['mode'])){
            ...
            $sql['merchant']=['in',get_auth_merchant()];//补全授权商户数据
            $sql=auth('gatherclass',$sql);//数据鉴权
            $arr = Gatherclass::with('merchantinfo,customerinfo,userinfo')->where($sql)->order('id desc')->select();//查询数据
            //判断报表类型
            if(empty($input['mode'])){
                //简易报表
                $formfield=get_formfield('gather_export','array');//获取字段配置
                //开始构造导出数据
                $excel=[];//初始化导出数据
                //1.填充标题数据
                ...
                //详细报表
                $files=[];//初始化文件列表
                $formfield=get_formfield('gather_exports','array');//获取字段配置
                //循环CLASS数据
                foreach ($arr as $arr_vo) {
                    $excel=[];//初始化导出数据
                    //1.填充标题数据
    ...
    public function prints(){
        $input=input('get.');
        if(isset_full($input,'id')){
            $print_name='gather';//模板标识
            $class=Gatherclass::where(['id'=>$input['id']])->find();
            $info=Gatherinfo::where(['pid'=>$input['id']])->select();
            $print_paper=get_sys(['print_paper']);
            //1.获取字段信息
            $formfield=get_formfield('gather_print','array');//获取字段配置
            //2.构造表格数据
